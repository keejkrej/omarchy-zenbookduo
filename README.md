...
- Fn row on the USB keyboard: media keys by default (mute / volume / brightness), hold Fn for F1–F12. Fn+Esc toggles that so F1–F12 become the default. F4 cycles keyboard backlight. Mic/F9 and Ctrl+Super are hold-to-talk dictation (Duo USB keyboard only). Emoji key opens the Omarchy emoji picker.
